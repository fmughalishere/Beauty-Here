...
const Profile = () => {
    ...
    if (!user) {
      ...
      const fetchUserOrders = async () => {
        try {
          ...
          if (token) {
            const response = await axios.get(`${API_BASE_URL}/api/order/user-orders`, {
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await axios.put(`${API_BASE_URL}/api/user/update`, editedUser, {
        headers: { Authorization: `Bearer ${token}` }
      });
        ...
        <aside className="profile-sidebar">
          <div className="sidebar-header">
            <div className="profile-avatar">
              {user.avatar ? (
                <img src={resolveImageUrl(user.avatar)} alt="Avatar" />
              ) : (
...
const ProfileSettingsSection = ({ user, isEditing, editedUser, handleChange, handleSave, setIsEditing, setUser }) => {
  ...
  const handleSaveChanges = async () => {
    if (avatarFile) {
      ...
      try {
        const token = localStorage.getItem('auth-token');
        const uploadRes = await axios.post(`${API_BASE_URL}/api/user/upload-avatar`, formData, {
          headers: {
        ...
        if (uploadRes.data.success) {
          setUser(prevUser => ({ ...prevUser, avatar: uploadRes.data.avatar }));
        }
      } catch (err) {
      ...
      {isEditing ? (
        <div className="profile-edit-form">
          ...
          <div className="form-actions">
            ...
            <button className="cancel-btn" onClick={handleCancelEditing}><FaTimes /> Cancel</button>
          </div>
        </div>
      ) : (
