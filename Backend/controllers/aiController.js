...
const generateSkincareRoutine = async (req, res) => {
    try {
        ...
        console.error("Google Gemini API Error (Routine):", error);
        res.status(500).json({ success: false, message: "Failed to generate routine. Please try again." });
    }
};

const chatWithGlo = async (req, res) => {
    try {
        const { prompt } = req.body;

        if (!prompt) {
            return res.status(400).json({ success: false, message: "Aap ne koi sawaal nahi poocha." });
        }
        ...
        res.status(500).json({ success: false, message: "AI se baat karte waqt masla hua." });
    }
};

export { generateSkincareRoutine, chatWithGlo };